...
internal object PromptContextFormatter {
    ...
    fun formatForCodePrefix(request: ProviderRequest): String = buildString {
        ...
        val packedContext = request.packedContextSummary.orEmpty()
        ...
        if ((inlineContext.isNotBlank() || retrievedContext.isNotBlank()) && packedContext.isNotBlank()) appendLine()
        if (packedContext.isNotBlank()) append(packedContext)
    }

    fun formatForInstructionPrompt(request: ProviderRequest): String = buildString {
        val inlineContext = request.inlineContext
